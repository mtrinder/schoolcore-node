...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'schoolcore-lib': '^' + bitcore.version,
    'schoolcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/schoolcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the schoolcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the schoolcore-node directory and configuration
